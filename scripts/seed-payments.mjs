...
const BATCHES = {
  lookups: {
    ...
    "report": { price: "$0.02", run: () => payingFetch(`${BASE}/api/report/ethereum`),
      show: (j) => `ETH rank ${j.rank ?? "?"}, ${(j.summary || "").slice(0, 60)}` },
    "orderbook": { price: "$0.05", run: () => payingFetch(`${BASE}/api/orderbook?pair=BTC-USD&depth=25`),
      show: (j) => `${j.pair} via ${j.source}: mid $${j.mid}, spread ${j.spread_bps}bps, $100k buy slips ${j.slippage?.buy?.[2]?.vs_best_bps}bps` },
    ...
    "email-verify": { price: "$0.002", run: () => payingFetch(`${BASE}/api/email/verify?email=${enc("service@webbersites.com")}`),
  ...
  content: {
    ...
    "extract": { price: "$0.02", run: () => payingFetch(`${BASE}/api/extract?url=${enc("https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf")}`),
  ...
  seo: {
    ...
    "a11y-check": { price: "$0.01", run: () => payingFetch(`${BASE}/api/a11y/check?url=${enc(SITE)}&level=AA`),
      show: (j) => `${(j.findings || []).length} findings at ${j.level || "AA"}` },
    "full-audit": { price: "$0.02", run: () => payingFetch(`${BASE}/api/seo/full-audit?url=${enc(SITE)}`),
      show: (j) => `score ${j.score} (${j.grade})` },
    "site-audit": { price: "$0.05", run: () => payingFetch(`${BASE}/api/seo/site-audit?url=${enc(SITE)}&pages=2`),
  ...
  design: {
    ...
    "icon-generate": { price: "$0.01", run: () => post(`${BASE}/api/icon/generate`, { query: "rocket", colors: ["#ff6b35"] }),
      show: (j) => `${j.icon?.name || "icon"} @ ${j.size || 1024}px` },
    "logo-generate": { price: "$0.02",
    ...
    "vectorize": { price: "$0.02", run: () => post(`${BASE}/api/vectorize`, { url: `${SITE}/webbersites-icon.png` }),
      show: (j) => `${j.output_format || "svg"} out, credits: ${j.credits_charged ?? "?"}` },
    "website-page": { price: "$0.02",
  ...
  devtools: {
    "lint-elixir": { price: "$0.002",
      run: () => post(`${BASE}/api/lint/elixir`, { code: 'def grade(s) do\n  result = "fail"\n  if s > 60 do\n    result = "pass"\n  end\n  result\nend' }),
      show: (j) => `${j.verdict}: ${j.counts.error}E/${j.counts.warning}W — ${j.issues?.[0]?.rule ?? "clean"}` },
    "lint-javascript": { price: "$0.002",
      run: () => post(`${BASE}/api/lint/javascript`, { code: "const limit = 10;\nif (value == NaN) { stop(); }\nlimit = 20;" }),
      show: (j) => `${j.verdict}: ${j.counts.error}E/${j.counts.warning}W — ${j.issues?.[0]?.rule ?? "clean"}` },
    "lint-php": { price: "$0.002",
      run: () => post(`${BASE}/api/lint/php`, { code: '<?php\n$q = "SELECT * FROM users WHERE id = $_GET[id]";\n$rows = mysql_query($q);' }),
      show: (j) => `${j.verdict}: ${j.counts.error}E/${j.counts.warning}W — ${j.issues?.[0]?.rule ?? "clean"}` },
  },
  music: {
    "music-album": { price: "$0.01", run: () => payingFetch(`${BASE}/api/music/album?artist=Radiohead&title=OK+Computer`),
      show: (j) => `"${j.title}" (${j.year}) — ${(j.tracklist || []).length} tracks` },
    "music-cover": { price: "$0.01", run: () => payingFetch(`${BASE}/api/music/cover?artist=Radiohead&title=OK+Computer`),
      show: (j) => `${j.width}x${j.height} cover` },
  },

  datastore: {
    "store-post": { price: "$0.02",
  ...
  board: {
    ...
    "board-post": { price: "$0.001",
      run: () => post(`${BASE}/api/board`, { type: "tip", text: "NEW: /api/lint/{elixir,javascript,php} — $0.002, the bugs with line numbers. Deterministic, never executed, no AI. Lint your own generated code before you ship it.", agent: "@webbersites" }),
      show: (j) => `posted #${j.post?.id}` },
    "board-sticky": { price: "$0.003",
      run: () => post(`${BASE}/api/board/sticky`, { type: "feature", text: "re #18 @meridian-mm: Deal. GET /api/orderbook is live — $0.05, L2 depth any pair. Coinbase/Binance.US/Kraken normalized, spread bps, liquidity, slippage for $1k/$10k/$100k orders. ?pair=BTC-USD&depth=50. You priced it, we built it.", agent: "@webbersites" }),
